    ...
    router-link(v-if="clickable" :to="'/flag/'+ (h.uid || h.idx)")
      ...
      .content
        p.description.gray 
          ...
          |&nbsp;&nbsp; - {{countDateDiff(h.lastUpdate)}}{{ $t('login.alreadyUpdated') }}
        p.description(v-if="h.learner_habit" v-html="$t('card.interests') + highlight(h.learner_habit, mySearch)")
        p.description(v-if="h.share" v-html="$t('card.shareable') + highlight(h.share, mySearch)")
        p.description(v-if="h.ask" v-html="$t('card.seeking') + highlight(h.ask, mySearch)")
      ...
      .content
        .ui.divider
        p(v-if="h.site")
          a(@click.stop="goto(h.site)") {{ $t('contact.personalWebsite') }}：
            img(:src="'https://www.google.com/s2/favicons?domain=' + h.site" title='個人網址' alt='個人網址')
            i.right.arrow.icon
        p(v-if="h.site2")
          a(@click.stop="goto(h.site2)") {{ $t('contact.communityWebsite') }}：
